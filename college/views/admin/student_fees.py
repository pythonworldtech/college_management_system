...
#=================================== Admin View Student Fess =============================

def admin_view_student_fees(request):
          if request.session.has_key('email'):
                    ...
                    return render(request, 'admin/view_student_fees.html',data)
          else:
...
def admin_view_student_fees_receipt(request):
          if request.session.has_key('email'):
                    email = request.session["email"]
                    admin = Login.objects.filter(email=email)
                    ...
                    return render(request, 'admin/admin_view_student_fees_receipt.html',data)
          else:
                    return redirect('login')
...
def admin_student_fees_receipt(request,pk):
          if request.session.has_key('email'):
                    receipt = Student_fees_Receipt.objects.filter(id=pk)
                    data = {
                              'receipt':receipt
                    }
                    return render(request, 'admin/fee_receipt.html',data)
          else:
                    return redirect('login')
