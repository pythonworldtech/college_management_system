...
def admin_add_student(request):
          if request.session.has_key('email'):
                    ...
                    if request.method=="POST":
                              ...
                              return render(request, 'admin/add_student.html',data)
          else:
...
def admin_manage_student(request):
  if request.session.has_key('email'):
        ...
        return render(request, 'admin/manage_student.html',data)
  else:
    return redirect('login')
...
def admin_view_student(request,pk):
  if request.session.has_key('email'):
        email = request.session["email"]
        admin = Login.objects.filter(email=email)
...
def admin_edit_student(request,pk):
    if request.session.has_key('email'):
        ...
        if request.method=="POST":
            ...
            return redirect(f"/admin_edit_student{pk}")
...
#=================================== Admin Delete Student Picture ===========================

def admin_delete_student_picture(request,pk):
    if request.session.has_key('email'):
        admin = Login.objects.filter(id=pk)
        for c in admin:
            id=c.id
            ...
            return redirect(f"/admin_edit_student{id2}")
    else:
        return redirect('login')
